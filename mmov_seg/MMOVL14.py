...
class MMOV(nn.Module):
    ...
    def __init__(
        ...
        self.upsample1 = nn.ConvTranspose2d(self.proj_dim, 256, kernel_size=2, stride=2) if self.use_clip and \
                                                                                            self.clip_decod_dim[
                                                                                                0] != 0 else None
        self.upsample2 = nn.ConvTranspose2d(self.proj_dim, 128, kernel_size=4, stride=4) if self.use_clip and \
                                                                                            self.clip_decod_dim[
                                                                                                1] != 0 else None

        self.dino_decod_proj1 = nn.ConvTranspose2d(in_channels=self.proj_dim, out_channels=256, kernel_size=2,
                                                   stride=2) if self.dino_model and self.dino_decod_dim[0] != 0 else None
        self.dino_decod_proj2 = nn.ConvTranspose2d(in_channels=self.proj_dim, out_channels=128, kernel_size=4,
                                                   stride=4) if self.dino_model and self.dino_decod_dim[
            0] != 0 else None

        self.dino_down_sample = nn.Conv2d(in_channels=self.proj_dim, out_channels=768, kernel_size=1, stride=1,
                                          padding=0) if self.dino_model else None
        #####################################################################################################################
        self.dino_sardecod_proj1 = nn.ConvTranspose2d(in_channels=self.proj_dim, out_channels=256, kernel_size=2,
                                                   stride=2) if self.dino_model and self.dino_decod_dim[0] != 0 else None
        self.dino_sardecod_proj2 = nn.ConvTranspose2d(in_channels=self.proj_dim, out_channels=128, kernel_size=4,
                                                      stride=4) if self.dino_model and self.dino_decod_dim[
            0] != 0 else None
        self.dino_sardown_sample = nn.Conv2d(in_channels=self.proj_dim, out_channels=768, kernel_size=1, stride=1,
                                             padding=0) if self.dino_model else None
        #####################################################################################################################
        self.layer_indexes = [3, 7] if clip_pretrained == "ViT-B/16" or clip_pretrained == "RemoteCLIP-ViT-B-32" else [
            7, 15]
        self.layers = []
        self.layers1 = []
        if self.use_clip:
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.SEM_SEG_HEAD.USE_DINO_CORR:
            ...
            dino = torch.hub.load(
                ...
                model=model_name,
                source="local", weights='./checkpoint/dinov3_vitl16_pretrain_sat493m-eadcf0ff.pth'
            )
            ...
            #################################################
            dino_sar = torch.hub.load(
                repo_or_dir=DINOV3_LOCATION,
                model=model_name,
                source="local", weights='./checkpoint/dinov3_vitl16_pretrain_sat493m-eadcf0ff.pth'
            )

            state_dict = torch.load('./checkpoint/L14_checkpoint.pth', map_location='cpu')
            ...
            msg = dino_sar.load_state_dict(state_dict, strict=False)
            dino_sar = dino_sar.float()

            for name, params in dino_sar.named_parameters():
                if dino_ft == "attention":
                    if "attn.qkv.weight" in name:
                        params.requires_grad = True
                    elif "pos_embed" in name:
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            sarimages = [x["sarimage"].to(self.device) for x in batched_inputs]  #######################
            ...
            clip_images_resized = F.interpolate(clip_images.tensor, size=self.clip_resolution, mode='bilinear',
                                                align_corners=False, )
            dino_images_resized = F.interpolate(clip_images.tensor, size=self.dino_resolution, mode='bilinear',
                                                align_corners=False, )
            ################################################
            clip_sarimages = [(x - self.clip_pixel_mean) / self.clip_pixel_std for x in sarimages]
            clip_sarimages = ImageList.from_tensors(clip_sarimages, self.size_divisibility)
            ################################################
            dino_sarimages_resized = F.interpolate(clip_sarimages.tensor, size=self.dino_resolution, mode='bilinear',
                                                   align_corners=False, )
            ...
            with torch.no_grad():
                ...
                clip_sarimages = [(x - self.clip_pixel_mean) / self.clip_pixel_std for x in sarimages]
                clip_sarimages = ImageList.from_tensors(clip_sarimages, self.size_divisibility)
                dino_sarimages_resized = F.interpolate(clip_sarimages.tensor, size=self.dino_resolution,
            ...
            with torch.no_grad():
                ...
                image = F.interpolate(images[0].unsqueeze(0), size=out_res, mode='bilinear',
                ...
                clip_images_resized = F.interpolate(clip_images, size=self.clip_resolution, mode='bilinear',
                                                    align_corners=False, )
                dino_images_resized = F.interpolate(clip_images, size=self.dino_resolution, mode='bilinear',
                                                    align_corners=False, )
                self.layers = []
                self.layers1 = []

        if self.dino_model is not None:
            dino_feat = self.dino_model.get_intermediate_layers(dino_images_resized,
                                                                n=24)  
            dino_patch_feat_last_unfold = rearrange(dino_feat[-1], "B (H W) C -> B C H W", H=24)
            dino_feat_down = self.dino_down_sample(dino_patch_feat_last_unfold)  # B,512,24,24
            dino_feat_L4 = rearrange(dino_feat[7], "B (H W) C -> B C H W", H=24)
            dino_feat_L8 = rearrange(dino_feat[15], "B (H W) C -> B C H W", H=24)
            ...
            ################################################
            dino_sarfeat = self.dino_sarmodel.get_intermediate_layers(dino_sarimages_resized,
                                                                      n=24)  
            dino_sarpatch_feat_last_unfold = rearrange(dino_sarfeat[-1], "B (H W) C -> B C H W", H=24)
            dino_sarfeat_down = self.dino_sardown_sample(dino_sarpatch_feat_last_unfold)  
            dino_sarfeat_L4 = rearrange(dino_sarfeat[7], "B (H W) C -> B C H W", H=24) 
            dino_sarfeat_L8 = rearrange(dino_sarfeat[15], "B (H W) C -> B C H W", H=24) 

            dino_sarfeat_L4_proj = self.dino_sardecod_proj1(
                dino_sarfeat_L4) if self.dino_sardecod_proj1 is not None else None
            dino_sarfeat_L8_proj = self.dino_sardecod_proj2(
                dino_sarfeat_L8) if self.dino_sardecod_proj2 is not None else None
            # dino_sarfeat_guidance = [dino_sarfeat_L4_proj, dino_sarfeat_L8_proj]
            ###################### Fusion ##########################
            dino_feat_down = dino_feat_down + dino_sarfeat_down
            dino_feat_L4_proj = dino_feat_L4_proj + dino_sarfeat_L4_proj
            dino_feat_L8_proj = dino_feat_L8_proj + dino_sarfeat_L8_proj
        ...
        if self.training:
            ...
            outputs = F.interpolate(outputs, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear",
                                    align_corners=False)
