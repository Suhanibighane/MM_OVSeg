...
def BuildRSIB(Weights):
    ...
    if os.path.isfile(Weights):
        ...
        for checkpoint_key in ["teacher", "model"]:
            if checkpoint_key in state_dict:
                print(f"Take key '{checkpoint_key}' in provided checkpointr dict")
...
class MMOV(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        #####################################################################################################################
        self.dino_sardecod_proj1 = nn.Conv2d(in_channels=768, out_channels=256, kernel_size=1, stride=1,padding=0) if self.dino_model and self.dino_decod_dim[0] != 0 else None
        self.dino_sardecod_proj2 = nn.ConvTranspose2d(in_channels=768, out_channels=128, kernel_size=2,stride=2) if self.dino_model and self.dino_decod_dim[0] != 0 else None
        self.dino_sardown_sample = nn.Conv2d(in_channels=768, out_channels=512, kernel_size=2, stride=2,padding=0) if self.dino_model else None
        #####################################################################################################################
    ...
    def from_config(cls, cfg):
        ...
        if cfg.MODEL.SEM_SEG_HEAD.USE_DINO_CORR:
            ...
            dino = BuildRSIB("./checkpoint/RSIB.pth")
            ...
            #################################################
            dino_sar = BuildRSIB("./checkpoint/B16_checkpoint.pth")
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ################################################
            dino_sarimages_resized = F.interpolate(clip_sarimages.tensor, size=self.dino_resolution, mode='bilinear',align_corners=False, )
        ...
        if self.dino_model is not None:
            ...
            ################################################
            dino_sarfeat = self.dino_sarmodel.get_intermediate_layers(dino_sarimages_resized,n=12)  # actually only 12 layers, but use a large num to avoid ambiguity
            dino_sarpatch_feat_last_unfold = rearrange(dino_sarfeat[-1][:, 1:, :], "B (H W) C -> B C H W", H=48)
            dino_sarfeat_down = self.dino_sardown_sample(dino_sarpatch_feat_last_unfold)  # B,512,24,24
            dino_sarfeat_L4 = rearrange(dino_sarfeat[3][:, 1:, :], "B (H W) C -> B C H W", H=48)
            dino_sarfeat_L8 = rearrange(dino_sarfeat[7][:, 1:, :], "B (H W) C -> B C H W", H=48)

            dino_sarfeat_L4_proj = self.dino_sardecod_proj1(dino_sarfeat_L4) if self.dino_sardecod_proj1 is not None else None
            dino_sarfeat_L8_proj = self.dino_sardecod_proj2(dino_sarfeat_L8) if self.dino_sardecod_proj2 is not None else None
        ...
        outputs = self.sem_seg_head(clip_features,dino_feat_down, clip_features_guidance, dino_feat_guidance)  #, files_name)
        #visualize_corr(outputs, files_name[0], save_prefix='./vis_cost_out_final/')
        if self.training:
